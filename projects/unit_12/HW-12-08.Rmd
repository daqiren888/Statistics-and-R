...
df3 <- df %>%
  mutate(tviews = sign(rate) *  log2(abs(views) + 1))
...
df3 <- df3 %>%
  mutate(trate = sign(rate) * log2(abs(rate) + 1 ))
...
df3 <- df3 %>%
   mutate(tlength = log(abs(length) + 1))
#  mutate(tlength =  sign(rate) * log(abs(length) + 1))
...
# e <- resid(f5)
# f6 <- lm( df3$tviews ~ df3$trate + df3$tlength, weights = 1/abs(e))
# summary(f6)
