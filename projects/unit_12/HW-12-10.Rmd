...
df3 <- df %>%
  mutate(tviews = log2(abs(views) + 1))
...
ggarrange(p21, p22, p23, labels = c("G", "H", "I" ), ncol = 2, nrow =2)
...
par(mfrow=c(2,2))
plot(f5, which =1)
plot(df3$tviews,f5$residuals)
mean(f5$residuals)
plot(df3$trate,f5$residuals)
plot(df3$tlength,f5$residuals)
