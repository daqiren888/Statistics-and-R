...
fit2 <- lm(d2$logvotes ~ d2$logdisb  + d2$can_party)
summary(fit2)
...
b <- boxcox(logvotes ~ logdisb + can_party, data = d2)
#b
lambda <- b$x
lik <-b$y
bc<-cbind(lambda, lik) 
...
c1 <- lm(lamvotes ~  logdisb + can_party)
summary(c1)
e <- resid(c1)
c2 <- lm(e^2 ~ logdisb + can_party + I(logdisb^2) + I(can_party^2) + I(logdisb*can_party))
(R2 <- summary(c2)$r.sq)
(n <- nrow(c2$model))
(m <- ncol(c2$model))
(W <- n*R2)
...
c3 <- lm(lamvotes ~  logdisb + can_party, weights = 1/abs(e))
