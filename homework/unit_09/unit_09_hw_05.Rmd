...
#d1$prediction <- predict.lm(model, d1, interval = 'prediction', level = 0.95)
#d2$prediction <- predict.lm(model, d2, interval = 'prediction', level = 0.95)
#d3$prediction <- predict.lm(model, d3, interval = 'prediction', level = 0.95)
...
#print(df1)
#print(df2)
#predict.lm(model, newdata=data.frame(df$score(55.87, 68.83), df$length(19.81, 19.81)),  data = df, interval = 'prediction', level = 0.95)
...
#dat <- df %>% modelr::add_predictions(model)

#dat <- df %>% modelr::add_predictions(model) %>% modelr::add_residuals(model)
