...
- 1: Does the data of each of the two groups obey a normal distribution?
...
$$Null\ Hypothesis\ H_0: p=0.5$$ Someone in home infected by COVID-19 has      equal probability to approve or disapprove the governor.  
$$Alternative\ Hypothesis\ H_1: p≠0.5$$  Someone in their home infected |       by COVID-19 is more likely to approve or disapprove the governor.

\
A two-sided research hypothesis is used and, thus, the research hypothesis H0 indicates that someone in home infected by COVID-19 has equal probability to approve or disapprove the governor. The alternative hypothesis H1 indicates that someone in their home infected by COVID-19 is more likely to approve or disapprove the governor. The procedure for the test involves pooling the observations from the two samples into one combined sample, keeping track of which sample each observation comes from, and then ranking lowest to highest from, respectively.
...
In the above result, the p value is the significance level of the Wilcoxon test (p-value = 0.04289). The p-value for the test is 0.04289, which is less than the significance level alpha = 0.05. We can conclude that who have had someone in their home infected by COVID-19 more likely to disapprove of the way their governor is handling the pandemic.


### Practical Significance 
I use Spearman rank correlation coefficient to test the practical significance. Spearman’s rho statistic is often used to estimate a rank-based measure of association, and used if the data do not come from a bivariate normal distribution. In the result below: S is the value of the test statistic (S = 9.1505e+10); p-value is the significance level of the test statistic (p-value = 0.4288). For Spearmann correlation coefficient it’s rho (Cor.coeff =-0.02246033). The rho-value of the test is -0.02246033, i.e. |rho| << 0.1,  we can conclude that the Covid-19 infection and governor approval are almost not correlated. In other words, it is not practically significant. 
...
We have conducted this test based on the data available in the ANES. However, even after an appropriate analysis of the data on hand, it shows only a medium correlation to support the conclusion that who have had someone in their home infected by COVID-19 more likely to disapprove of the way their governor is handling the pandemic. People disapprove their governor could because of many different reasons. If this were to be the case, then our study design couldn't find all the causes. Use more  data items to analysis could be helpful to give a more accurate result about the Covid-19 inspections.  
